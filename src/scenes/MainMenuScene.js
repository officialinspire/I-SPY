...
export default class MainMenuScene extends Phaser.Scene {
  ...
  showNotice(message) {
    ...
    this.time.delayedCall(2600, () => this.notice.setVisible(false));
  }

  layout(gameSize) {
    const { width, height } = gameSize;
    const compact = width < 520;
    this.title.setFontSize(compact ? 54 : 72).setPosition(width / 2, Math.max(92, height * 0.17));
    this.subtitle.setPosition(width / 2, this.title.y + 58).setWordWrapWidth(Math.min(520, width - 36));
    const startY = Math.max(this.subtitle.y + 72, height * 0.40);
    const spacing = compact ? 54 : 58;
    this.buttons.forEach((button, index) => button.setPosition(width / 2, startY + index * spacing));
    this.status.setPosition(width / 2, height - 24);
